...
class RobotAgent(Agent):
    ...
    def update_path(self, target):
        ...
        #usamos A* para calcular la ruta 
        print(f"Robot {self.unique_id}: busca ruta de {self.pos} a {target}...")
        ...
        if not self.path:
            print(f"Robot {self.unique_id}: SIN RUTA") 
        else:
            print(f"Robot {self.unique_id}: Ruta para {len(self.path)} pasos.") 
...
#ASIGNACIONES DE PRODUCTOS PICKUP / DROPOFF
    def assign_new_pickup(self):
        self.current_pickup = self.model.get_random_free_cell()
        print(f"Robot {self.unique_id}: Nuevo Pickup en {self.current_pickup}") 
    ...
    def assign_new_dropoff(self):
        ...
        print(f"Robot {self.unique_id}: Nuevo Dropoff en {self.current_dropoff}") 
    ...
    def step(self):
        ...
        if self.path:
            ...
            if not agents_in_cell:
                ...
                print(f"Robot {self.unique_id}: Esperando... celda {next_pos} ocupada.") 
        
        if self.current_pickup and self.pos == self.current_pickup:
            self.model.remove_target_at(self.current_pickup)
            self.current_pickup = None
            print(f"Robot {self.unique_id}: ¡Recogió paquete!") 
            ...
            print(f"Robot {self.unique_id}: ¡ENTREGADO! Total: {self.deliveries_count}") 
